...
public class Calculator {
	...
	// 리턴타입 있는 것
	// 메소드의 오버로딩 > 동일한 이름의 메소드(리턴타입 상관x, 매개변수 타입 또는 개수가 달라야 함)
	...
	// 밑변과 높이를 입력 받아 > 삼각형의 넓이 계산 메소드. getTriangleArea 리턴타입 더블
	double getTriangleArea(double width, double height) {
		return width*height/2;
	}
	
	
	// 매개변수 클래스, 배열 올 수 있음 (배열 값 모두 더하는 기능)
	int sum(int[] ary) {
		int sum = 0;
	...
	// 매개변수의 개수 상관없이 받아오는 방법 (타입... args) / 계산은 배열처럼
